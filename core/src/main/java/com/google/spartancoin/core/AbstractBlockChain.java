 ...
 * the lightweight SPV mode, a {@link com.google.spartancoin.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.google.spartancoin.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     */
    private void DigiShieldRetarget(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException 
    {
        checkState(lock.isHeldByCurrentThread());
        Block prev = storedPrev.getHeader();
        int retargetInterval = CoinParams.getInterval(storedPrev.getHeight(), params.getId().equals(NetworkParameters.ID_TESTNET)) ;
        
        // Is this supposed to be a difficulty transition point?
        // if ((storedPrev.getHeight() + 1) % params.getInterval(storedPrev.getHeight(), params.getId().equals(NetworkParameters.ID_TESTNET)) != 0) {
        ...
        if (timespan < (targetTimespan - (targetTimespan/4)) ){ timespan = (targetTimespan - (targetTimespan/4));}
        if (timespan > (targetTimespan + (targetTimespan/2)) ){ timespan = (targetTimespan + (targetTimespan/2));}
        ...
        verifyDifficulty(newDifficulty, storedPrev, nextBlock, "DigiShieldRetarget");
    }
   ...
   /* current difficulty formula, megacoin - kimoto gravity well */
        ...
        {
            verifyDifficulty(params.getProofOfWorkLimit(), storedPrev, nextBlock, "KGW");
            return;
        }
        ...
        verifyDifficulty(newDifficulty, storedPrev, nextBlock, "KGW");

    }
    private void verifyDifficulty(BigInteger calcDiff, StoredBlock storedPrev, Block nextBlock, String funcStr)
    {
        ...
        calcDiff = calcDiff.and(mask);
          if (calcDiff.compareTo(receivedDifficulty) != 0)
              throw new VerificationException(funcStr + ": Network provided difficulty bits do not match what was calculated: " +
    ...
    private void checkDifficultyTransitions_V2(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        final long BlocksTargetSpacing = 2 * 60; // 1 minutes
        int   TimeDaySeconds = 60 * 60 * 24;
        long  PastSecondsMin = (int)(TimeDaySeconds * 0.25);
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
      checkState(lock.isHeldByCurrentThread());
      if (storedPrev.getHeight()+1 > CoinParams.nDiffChangeBlock) {  DigiShieldRetarget(storedPrev, nextBlock); }
      else{
